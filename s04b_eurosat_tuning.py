...
OUTPUT_DIR = Path('/kaggle/working/image_tuning_results')
...
t_start = time.time()
stack_scores = cross_val_score(stacking_clf, X_train, y_train, cv=StratifiedKFold(n_splits=3, shuffle=True, random_state=RANDOM_SEED), scoring='f1_macro', n_jobs=1)
...
rf_results_df.to_csv(OUTPUT_DIR / 'rf_grid_search_image.csv', index=False)
...
xgb_results_df.to_csv(OUTPUT_DIR / 'xgb_grid_search_image.csv', index=False)
