...
OUTPUT_DIR = Path('/kaggle/working/rf_results')
...
checkpoint_file = CHECKPOINT_DIR / 'rf_progress.json'
results_file = CHECKPOINT_DIR / 'rf_results_intermediate.csv'
...
for noise_type, noise_level, fold_idx in remaining_runs:
    ...
    X_train_fold, X_val_fold, y_train_fold_clean, y_val_fold, _, _ = get_fold_data(
...
results_df.to_csv(OUTPUT_DIR / 'rf_results_full.csv', index=False)
...
summary.to_csv(OUTPUT_DIR / 'rf_summary.csv', index=False)
...
per_class_summary.to_csv(OUTPUT_DIR / 'rf_per_class_metrics.csv', index=False)
...
nsi_df.to_csv(OUTPUT_DIR / 'rf_nsi.csv', index=False)
...
pd.DataFrame([test_result_record]).to_csv(OUTPUT_DIR / 'rf_test_results.csv', index=False)
