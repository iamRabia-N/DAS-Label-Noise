...
OUTPUT_DIR = Path('/kaggle/working/voting_image_results')
...
def build_voting_classifier(rf_params, xgb_params, xgb_base, et_params, random_state, gpu_available):
    rf = RandomForestClassifier(
        **rf_params, n_jobs=-1, random_state=random_state, class_weight='balanced_subsample'
    )
    xgb_full_params = {**xgb_base, **xgb_params, 'random_state': random_state}
    if gpu_available:
        xgb_full_params['device'] = 'cuda'
    else:
        xgb_full_params.pop('device', None)
    xgb = XGBClassifier(**xgb_full_params)
...
checkpoint_file = CHECKPOINT_DIR / 'voting_image_progress.json'
results_file = CHECKPOINT_DIR / 'voting_image_results_intermediate.csv'
...
for noise_type, noise_level, fold_idx in remaining_runs:
    ...
    voting_clf = build_voting_classifier(
        best_rf_params, best_xgb_params, xgb_base_params, et_params,
        random_state=RANDOM_SEED + fold_idx,
        gpu_available=GPU_AVAILABLE
    )
    voting_clf.fit(X_train_fold, y_train_fold_noisy)
    ...
    del voting_clf, X_train_fold, X_val_fold, y_train_fold_clean, y_train_fold_noisy, y_val_fold, y_pred
    gc.collect()
...
results_df.to_csv(OUTPUT_DIR / 'voting_image_results_full.csv', index=False)
...
summary.to_csv(OUTPUT_DIR / 'voting_image_summary.csv', index=False)
...
per_class_summary.to_csv(OUTPUT_DIR / 'voting_image_per_class_metrics.csv', index=False)
...
nsi_df.to_csv(OUTPUT_DIR / 'voting_image_nsi.csv', index=False)

voting_final = build_voting_classifier(
    best_rf_params, best_xgb_params, xgb_base_params, et_params,
    random_state=RANDOM_SEED,
    gpu_available=GPU_AVAILABLE
)
voting_final.fit(X_train, y_train)
y_test_pred = voting_final.predict(X_test)
...
pd.DataFrame([test_result_record]).to_csv(OUTPUT_DIR / 'voting_image_test_results.csv', index=False)
