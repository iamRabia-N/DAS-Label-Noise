...
OUTPUT_DIR = Path('/kaggle/working/gate_results')
...
for p in prob_files:
    if p.name not in seen:
        seen.add(p.name)
        files.append(p)
assert files, "No probs_*.npz found - attach the s13a to s13c outputs"
...
df.to_csv(OUTPUT_DIR / 'gate_results_full.csv', index=False)
...
summary.to_csv(OUTPUT_DIR / 'gate_summary_by_condition.csv', index=False)
...
drops.to_csv(OUTPUT_DIR / 'gate_relative_drops.csv', index=False)
...
for nt in ['symmetric', 'asymmetric', 'pairflip']:
    deltas = noisy[noisy['noise_type'] == nt]['das_delta'].values
    if len(deltas) >= 5:
        stat, p = wilcoxon(deltas)
        wil_rows.append({'dataset': 'ALL_POOLED', 'noise_type': nt, 'n': len(deltas), 'mean_das_delta': round(float(deltas.mean()), 4), 'wilcoxon_p': round(float(p), 4)})
alld = noisy['das_delta'].values
stat, p = wilcoxon(alld)
wil_rows.append({'dataset': 'ALL_POOLED', 'noise_type': 'all', 'n': len(alld), 'mean_das_delta': round(float(alld.mean()), 4), 'wilcoxon_p': round(float(p), 4)})
wil = pd.DataFrame(wil_rows)
wil.to_csv(OUTPUT_DIR / 'gate_das_wilcoxon.csv', index=False)
...
spdf.to_csv(OUTPUT_DIR / 'gate_kappa_gap_spearman.csv', index=False)
