...
DATASETS = ['drybean', 'pendigits', 'satimage', 'shuttle']
...
OUTPUT_DIR = Path('/kaggle/working/base_probs')
...
CONDITIONS = [('clean', 0.0)] + [('symmetric', l) for l in [0.1, 0.2, 0.3, 0.4]] + [('asymmetric', l) for l in [0.1, 0.2, 0.3, 0.4]] + [('pairflip', 0.4)]

manifest_csv = OUTPUT_DIR / 'probs_manifest.csv'
prior_manifest = sorted(Path('/kaggle/input').rglob('probs_manifest.csv'))
manifest = pd.read_csv(prior_manifest[0]).to_dict('records') if prior_manifest else []
done_units = set((m['dataset'], m['condition'], m['fold']) for m in manifest)
...
for key in DATASETS:
    ...
    assert split_hits, f"{key}_splits.npz not found - attach the s10 output"
